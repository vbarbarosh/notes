...
# tor is installed but never started by the image; a job starts it on demand
# when YT_DLP_TOR is enabled. The packaged service stays disabled.
RUN apt-get update \
    && apt-get install -y --no-install-recommends python3-venv ffmpeg ca-certificates tor \
